...
**Black Squid** — the parent company / product hub for creative audio software.
Cloned from the original EZStemz site so it keeps the same battle-tested auth,
Stripe checkout, and paywalled-download stack, now rebranded as a multi-product
hub. Current products: **EZStemz** (live) and **KitForge** (coming soon).

- **Framework**: Next.js 16 (App Router) on React 19
- **UI**: Chakra UI v3 + `next-themes` — minimal dark theme (black canvas, white text)
- **Auth + DB**: Supabase (Postgres, `@supabase/ssr`)
- **Payments**: Stripe Checkout (one-time license, per product)
- **Distribution**: short-lived signed redirects to the latest GitHub Release
  asset (`.dmg` on macOS, `.exe` installer on Windows)

## Products

Products are registered in a single place: [`src/lib/products.ts`](src/lib/products.ts).

| Product  | Status      | Stripe                                                  |
| -------- | ----------- | ------------------------------------------------------- |
| EZStemz  | Available   | Live — uses `EZSTEMZ_STRIPE_PRICE_ID`                   |
| KitForge | Coming soon | Placeholder — `KITFORGE_STRIPE_PRICE_ID` unset, disabled |

### How EZStemz Stripe still works

The EZStemz purchase flow is intact. `/api/checkout` creates a Stripe Checkout
Session from `env.ezstemzPriceId` (`EZSTEMZ_STRIPE_PRICE_ID`), the webhook at
`/api/stripe/webhook` fulfils purchases into the `purchases` table, and
`BuyButton` drives the owns / signed-in / signed-out states. That button lives on
`/products/ezstemz` and `/pricing`.

> The only Stripe change from the original site is the env-var name:
> `STRIPE_PRICE_ID` → `EZSTEMZ_STRIPE_PRICE_ID` (namespaced per product).

### Adding the KitForge Stripe price_id later

1. Create the KitForge product + price in the Stripe Dashboard.
2. Put the price id in `KITFORGE_STRIPE_PRICE_ID` (see `.env.local.example`).
3. In `src/lib/products.ts`, set `kitforge.priceId` to
   `process.env.KITFORGE_STRIPE_PRICE_ID ?? null` and flip `enabled: true`.
4. Wire a KitForge branch into `/api/checkout` (or generalise it to accept a
   `productId`). The UI already switches the button from "Coming Soon" to active
   via `isCheckoutEnabled()`.

```
black-squid
├── src/
│   ├── app/                       Next.js routes
│   │   ├── page.tsx               Black Squid homepage (logo, hero, product cards)
│   │   ├── products/             Product hub
│   │   │   ├── page.tsx           "My Products" index
│   │   │   ├── ezstemz/           EZStemz product page (+ live BuyButton)
│   │   │   └── kitforge/          KitForge product page (coming soon)
│   │   ├── pricing/               Product pricing cards
│   │   ├── login/
│   │   ├── signup/
...
│   │   ├── site/                  Nav, footer, shell, logo, product-card, BuyButton
│   │   └── auth/                  AuthCard, LoginForm, SignupForm
│   ├── lib/
│   │   ├── env.ts                 Throwing env getters
│   │   ├── products.ts            Product catalogue (EZStemz + KitForge)
│   │   ├── theme.ts               Chakra v3 system + brand tokens (black/white)
...
└── supabase/
    └── config.toml                Local Supabase CLI config (schema lives in the linked project)
```

## 1. First-time install

```bash
cd ~/projects/black-squid
npm install
...
3. Schema: this repo ships **without** migration files — the `profiles` +
   `purchases` tables (and their RLS policies) already exist in the shared
   Supabase project that EZStemz used. Black Squid reuses that same project, so
   there's nothing to apply. If you point this at a fresh Supabase project,
   recreate the two tables + owner-only SELECT RLS by hand in the SQL Editor
   (all writes go through the service-role client, so end users only ever read
   their own rows).
...
RLS is enabled on `profiles` and `purchases` with a single SELECT policy each
(owner can read). All writes happen through the service-role client inside
route handlers, so end users can't forge a license row even if they bypass the
API.
...
2. **Products → + Add product**: e.g. _"EZStemz Desktop License"_, one-time
   price of $20 USD. Copy the **price ID** → `EZSTEMZ_STRIPE_PRICE_ID`.
   (When KitForge is ready, add a second product and set
   `KITFORGE_STRIPE_PRICE_ID`.)
...
Render assigns a URL like `https://black-squid.onrender.com` (or your
...
- Switch Stripe to live mode and re-register the webhook (signing secret is
  per-endpoint AND per-mode). Update `STRIPE_SECRET_KEY`, `EZSTEMZ_STRIPE_PRICE_ID`,
